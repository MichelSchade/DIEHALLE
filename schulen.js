...
function on_planyo_form_loaded(event) {
  jQuery('#price_info_div_sticky').css('background-color', '#7cc5b6');
  jQuery('.reservation_steps').hide();
  if (jQuery("#rental_prop_Sofortzahlung_oder_Terminanfrage option:selected").val() == "none") {
    jQuery(".reservation_step_5").hide();
    jQuery(".reservation_step_6").hide();
    jQuery(".reservation_step_6 > .reservation_step_name").text("Buchung abgeschlossen");
  }
  if (event == 'reserve') {
    jQuery("#rental_prop_Sofortzahlung_oder_Terminanfrage").change(function() {
      if (jQuery("#rental_prop_Sofortzahlung_oder_Terminanfrage option:selected").val() == "Sofortzahlung") {
        jQuery(".reservation_step_5").show();
        jQuery(".reservation_step_6").show();
        jQuery(".reservation_step_6 > .reservation_step_name").text("Buchung abgeschlossen");
      } else if (jQuery("#rental_prop_Sofortzahlung_oder_Terminanfrage option:selected").val() == "Terminanfrage") {
        jQuery(".reservation_step_6 > .reservation_step_name").text("Anfrage abgeschlossen");
        jQuery(".reservation_step_6").show();
        jQuery(".reservation_step_5").hide();
      } else {
        jQuery(".reservation_step_5").hide();
  ...
  if (jQuery("#step_reservation_details").hasClass('reservation_step_current')) {
    ...
    jQuery('#calendartype').hide();
  }
  if (jQuery('.reservation_step_2').hasClass('reservation_step_current')) {
    jQuery('.reservation_step_1').addClass('completedschool');
    jQuery('.completedschool .reservation_step_img_text').html('<img src="https://uploads-ssl.webflow.com/5e5bcb9d0e170635e182079a/5e9ff8e6f9a5b1d89d40f5cb_check-solid.svg" width=50% />')
  } else if (jQuery('.reservation_step_3').hasClass('reservation_step_current')) {
    jQuery('.reservation_step_1, .reservation_step_2').addClass('completedschool');
    jQuery('.completedschool .reservation_step_img_text').html('<img src="https://uploads-ssl.webflow.com/5e5bcb9d0e170635e182079a/5e9ff8e6f9a5b1d89d40f5cb_check-solid.svg" width=50% />')
  } else if (jQuery('.reservation_step_4').hasClass('reservation_step_current')) {
    jQuery('.reservation_step_1, .reservation_step_2, .reservation_step_3').addClass('completedschool');
    jQuery('.completedschool .reservation_step_img_text').html('<img src="https://uploads-ssl.webflow.com/5e5bcb9d0e170635e182079a/5e9ff8e6f9a5b1d89d40f5cb_check-solid.svg" width=50% />')
  } else if (jQuery('.reservation_step_5').hasClass('reservation_step_current')) {
    jQuery('.reservation_step_1, .reservation_step_2, .reservation_step_3, .reservation_step_4').addClass('completedschool');
    jQuery('.completedschool .reservation_step_img_text').html('<img src="https://uploads-ssl.webflow.com/5e5bcb9d0e170635e182079a/5e9ff8e6f9a5b1d89d40f5cb_check-solid.svg" width=50% />')
  }
  ...
  if (jQuery("#step_personal_details").hasClass('reservation_step_current') && jQuery("#rental_prop_Sofortzahlung_oder_Terminanfrage option:selected").val() == "Terminanfrage") {
    jQuery('#submit_button').val('Anfragen')
  }
  jQuery('#row_one_date').parent().addClass('hidefromcustomer');
  ...
  if (event == 'reserve_new_step') {
    ...
    jQuery('html, body').animate({
      scrollTop: jQuery("#planyo_content").offset().top
    }, 500);
    document.body.scrollTop = document.documentElement.scrollTop = 0;
    jQuery('.reservation_step_1, .reservation_step_2, .reservation_step_3').addClass('completed');
    jQuery('.completed .reservation_step_img_text').html('<img src="https://uploads-ssl.webflow.com/5e5bcb9d0e170635e182079a/5e9ff8e6f9a5b1d89d40f5cb_check-solid.svg" width=50% />');
